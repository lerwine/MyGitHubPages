...
(function (app) {
    ...
    function getDirectiveTemplatePath(directiveName, nameSpace) {
        if (typeof directiveName !== "string" || directiveName.trim().length == 0)
            throw new Error("Invalid directive name");
        if (arguments.length == 2 && !sys.isNil(nameSpace)) {
            if (typeof nameSpace !== "string" || nameSpace.trim().length == 0)
                throw new Error("Invalid namespace.");
            return DIRECTIVE_TEMPLATE_PATH + nameSpace + "/" + directiveName + ".htm";
        }
        if (typeof nameSpace === "string") {
            if (nameSpace.trim().length == 0)
                throw new Error("Invalid nameSpace.");
        }
        return DIRECTIVE_TEMPLATE_PATH + directiveName + ".htm";
    }
    app.getDirectiveTemplatePath = getDirectiveTemplatePath;
