...
(function (JsTokenType) {
    JsTokenType[JsTokenType["Unknown"] = 0] = "Unknown";
    JsTokenType[JsTokenType["Comment"] = 1] = "Comment";
    JsTokenType[JsTokenType["Identifier"] = 2] = "Identifier";
    JsTokenType[JsTokenType["Number"] = 3] = "Number";
    JsTokenType[JsTokenType["String"] = 4] = "String";
    JsTokenType[JsTokenType["OpenGroup"] = 5] = "OpenGroup";
    JsTokenType[JsTokenType["CloseGroup"] = 6] = "CloseGroup";
    JsTokenType[JsTokenType["Regex"] = 7] = "Regex";
    JsTokenType[JsTokenType["Terminator"] = 8] = "Terminator";
    JsTokenType[JsTokenType["NewLine"] = 9] = "NewLine";
    JsTokenType[JsTokenType["Operator"] = 10] = "Operator";
    JsTokenType[JsTokenType["WhiteSpace"] = 11] = "WhiteSpace";
})(JsTokenType = exports.JsTokenType || (exports.JsTokenType = {}));
class JsCodeParseContext {
        ...
        this._tokenParseInfo = [
            {
                ...
                match: {
                    ...
                    value: (options.allowBackquoteMultiline) ? function (matches) { return (matches[2] !== null) ? matches[2] : JSON.parse(matches[0]); } :
                        function (matches) { return JSON.parse(matches[0]); }
                }
            }, {
                regex: /^[\[{(]/,
                ...
                shouldParse: function (tokenChain) {
    ...
    ParseNextToken() {
        if (this._position >= this._code.length)
            return;
        let c = (this._position > 0) ? this._code.substr(this._position) : this._code;
        let tokenChain = undefined;
        for (var i = 0; i < this._tokenParseInfo.length; i++) {
            let parseInfo = this._tokenParseInfo[i];
            if (typeof (parseInfo.shouldParse) != "undefined" && !parseInfo.shouldParse(this._tokenChain))
                continue;
            let m = c.match(parseInfo.regex);
            if (m !== null) {
                let matchSpec = parseInfo.match;
                if (typeof (matchSpec) == "number")
                    ...
                    for (var n = 0; n < matchSpec.length; n++) {
                        let cg = matchSpec[n].codeGroup;
                        let vs = matchSpec[n].value;
                        let ng = (typeof (cg) == "undefined") ? [] : ((typeof (cg) == "number") ? [cg] : cg);
                        let vi = (typeof (vs) == "undefined") ? [] : ((typeof (vs) == "number") ? [vs] : vs);
                        if (ng.length == 0) {
                            if (!Array.isArray(vi)) {
                                tokenChain = {
                                    previous: this._tokenChain,
